...
class ShowD3SVG extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.routers !== prevProps.routers) {
      this.setState(this.setNodesLinks());
    }
  }

  link = (s, t) => ({ source: s, target: t, key: `${s}:${t}`, size: 2 });
  ...
  addNodesInCircle = (nodes, start, count, midX, midY, displace, rotate) => {
    ...
    rotate = rotate || 0;
    for (let i = start; i < start + count; i++) {
      a = Math.cos(((i - start) * 2.0 * Math.PI) / count + rotate);
      b = Math.sin(((i - start) * 2.0 * Math.PI) / count + rotate);
      nodes.push(this.node(i, midX + a * displace, midY + b * displace));
    }
  };
  setNodesLinks = () => {
    const nodes = [];
